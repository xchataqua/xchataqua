...
static NSUInteger numberOfColors = sizeof (def_color_vals) / sizeof (def_color_vals [0]);
...
- (void) load
{
	...
	for (NSUInteger i = 0; i < numberOfColors; i ++)
	{
		...
		colors[i] = [[UIColor alloc] initWithRed:def->r
										   green:def->g
											blue:def->b
										   alpha:1.0f];
	}
	...
	for (NSUInteger i = 0; i < numberOfColors; i++)
	{
		...
		[colors[i] release];
		colors[i] = [[UIColor alloc] initWithRed:r green:g blue:b alpha:a];
	}
}

- (void) save {
	/*
	 FIXME: rgb can be derived from cgcolor
	NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithCapacity:numberOfColors];
	for (NSUInteger i = 0; i < numberOfColors; i++)
	{
...
{
	if ( (self = [super init]) ) {
		colors = (UIColor **) malloc (numberOfColors * sizeof(UIColor *));
...
{
	return colors[color % numberOfColors];
}

- (void) setColor:(UIColor *)color forKey:(NSInteger)keyNumber
{
	[colors[keyNumber] release];
	colors[keyNumber] = [color retain];
}
