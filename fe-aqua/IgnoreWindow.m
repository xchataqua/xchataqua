...
@property (nonatomic, retain) NSNumber *ctcp, *private, *channel, *notice, *invite, *unignore;

@end

@implementation IgnoreItem
@synthesize ign, mask, ctcp, private, channel, notice, invite, unignore;
...
- (id) initWithIgnore:(struct ignore *)anIgnore
{
	if ((self = [super init]) != nil) {
		...
		self.private = [NSNumber numberWithBool:ign->type & IG_PRIV];
		self.channel = [NSNumber numberWithBool:ign->type & IG_CHAN];
...
- (void) awakeFromNib
{
	...
	for (NSUInteger i = 0; i < [self->ignoreTableView numberOfColumns]; i++) {
		[[[self->ignoreTableView tableColumns] objectAtIndex:i] setIdentifier:[NSNumber numberWithInteger:i]];
	}
...
- (void) tableView:(NSTableView *) aTableView
	...
	forTableColumn:(NSTableColumn *) aTableColumn
			   row:(NSInteger)rowIndex
{
	[[ignores objectAtIndex:rowIndex] setValue:anObject forField:[[aTableColumn identifier] integerValue]];
}
